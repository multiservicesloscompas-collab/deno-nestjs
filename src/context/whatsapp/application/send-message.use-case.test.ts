...
Deno.test("SendMessageUseCase splits long messages at word boundaries and capitalizes", async () => {
  ...
  const part1 = "esto es una prueba que sobrepasa los mil caracteres y debe ser cortada en un espacio. ".repeat(15); // > 1000
  const part2 = "esta es la segunda parte que tambien debe estar capitalizada.";
  ...
  for (const msg of sentMessages) {
    ...
    assertEquals(firstChar, firstChar.toUpperCase(), `Message should start with uppercase: ${msg.substring(0, 20)}...`);
  }
...
Deno.test("SendMessageUseCase sends short messages and capitalizes", async () => {
  const sentMessages: string[] = [];
